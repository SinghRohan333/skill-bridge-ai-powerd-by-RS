...
Return ONLY a valid JSON object with this exact structure, no markdown, no explanation, no code blocks:
{
  ...
  return { extractedData, gapData };
};

export const analyzeResumeWithJobDoc = async (resumeText, jobDocText) => {
  const completion1 = await groq.chat.completions.create({
    model: "llama-3.3-70b-versatile",
    messages: [
      {
...
Return ONLY a valid JSON object with this exact structure, no markdown, no explanation, no code blocks:
{
  ...
  const completion2 = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "user",
        content: `You are an expert career advisor. Analyze the following candidate's skills against the exact job description provided by the company and identify skill gaps.
...
Return ONLY a valid JSON object with this exact structure, no markdown, no explanation, no code blocks:
{
  "targetRole": "extract the job title from the job description or use Job Description",
  "matchScore": 75,
  "missingSkills": ["skill1", "skill2"],
  "strongSkills": ["skill1", "skill2"],
  "recommendations": [
    {
      "skill": "skill name",
      "reason": "why this skill is required based on the job description",
      "resource": "suggested learning resource"
    }
  ],
  "summary": "brief overall assessment based on the job description requirements"
}`,
      },
    ],
    temperature: 0.3,
